...
Si definisce $o_i$ la stima semplice (calcolata su una singola estrazione) dell'osservabile $O$ allo step $i$. La tecnica \textit{cluster jackknife} consiste nel ricavare le grandezze derivate $o^i$ definite come $$o^i=\langle o\rangle_{bin}+\tfrac{1}{N_{bin}-1}(\langle o\rangle_{bin}-o_i)$$ dove $N_{bin}$ indica il numero totale di $bin$.
