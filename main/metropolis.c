...
double W=1;
double D=3;
double x[N],o[N],tmp[N],var[N],gmm[30];
...
/*void selfCorrelation(double* x){
    int t; for(t = 0; t < 30-t; t++){
        int i; for(i = 0; i < cycles-t; i++)
...
int main(int argc,char* argv[]){
    ...
    /*init state*/
    int i,dt;for(i = 0; i < N; i++){
        x[i] = 100;
        o[i] = tmp[i] = var[i] = gmm[i] = 0;
    }
    /*init ranlux*/
    ...
    FILE *f4 = fopen("metro4.dat","w");
    /*metropolis loop*/
    for(i = 0; i < cycles; i++){
        metropolis(x);
        ooo(x,tmp);
        if((i+1)%BIN_W == 0){
            fprintf(f4,"%d",(i+1)/BIN_W);           /*print*/
            for(dt = 0; dt < N; dt++){
                o[dt] += tmp[dt]/BIN_W;
                fprintf(f4,"\t%lf",tmp[dt]/BIN_W);  /*print*/
